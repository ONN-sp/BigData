...
22. `pom.xml`中的`<scope>`：指的是依赖范围，test指测试的时候依赖，不会被打包到最终的jar包中
   ```xml
   <dependency>
   ...
   ```
23. <mark>provided指在运行时提供，不会被打包到最终的jar包中，运行时由运行环境提供，这个依赖由服务器 / 容器 / Flink 框架自己提供(我本地编译需要，但运行时别人已经提供了，你别打包进去)。如果要在本地运行代码，那么依赖的作用范围就是”编译+运行阶段“，因此必须用`<scope>compile</scope>`；而只是本地编译打jar包时，因为Flink 集群（Standalone / YARN / K8S）本身已经完整部署了 Flink 所有 Jar 包，所以`provided`就行</mark>
24. 依赖传递容易出现依赖冲突问题：在项目里面添加了两个不同的模块依赖，但是这两个依赖又依赖了不同版本的同一个jar包，就会出现依赖冲突（比如：模块a依赖了模块b和模块c，而模块b和模块c又依赖了一个不同版本的同一）。此时maven会根据一定规则来解决依赖冲突，比如：最短路径优先（路径层级少的优先使用）、先声明优先（当路径层级相同时，在`pom.xml`中先声明的优先使用）
    ![img.png](最短路径优先.png)
25. 手动解决依赖冲突：利用`<exclusion>`标签排除不需要的依赖，即在当前项目模块中的`pom.xml`中指定要排除的一个依赖，也就是我要用另一个依赖了不同版本的同一个jar包的依赖作为依赖（假设：模块a依赖了模块b和模块c，而模块b和模块c又依赖了一个不同版本的同一个jar包。此时手动解决就只需在模块a的pom文件中对模块b依赖时添加`<exclusion>`）。<optional>`标签指定是否可选依赖
26. <mark>父子工程：在一个实际项目中可能会有多个模块，这些模块之间可能会有依赖关系，如果每个模块都单独配置一遍这些依赖的话，会很麻烦，并且不利于统一管理，这种情况就可以用父子工程来管理公共依赖。父工程只需要将`pom.xml`中的`<packaging>`标签中的内容设置为`pom`即可，此时这个工程只是用来管理其它子工程的，不会生成任何`jar`和`war`包，因此可以直接删除`src`目录。创建子工程只需要在父工程鼠标右键新建模块即可</mark>
    ![img.png](父子工程.png)
27. 直接对父工程进行编译、打包`mvn clean 、mvn compile`等操作会对所有子工程进行对应的操作，即父工程有聚合的作用
28. 常见的做法是：
    * 父工程：
...
29. 依赖继承：子工程会继承父工程的依赖，即子工程不需要再配置依赖，直接使用即可，也就是父工程中的`<dependencies>`中的依赖子工程是完全不用写的；而`<dependencyManagement>`里的，所以子工程必须自己写依赖，但可以不写 version
30. 私服仓库的作用：
    * 作为中央仓库的代理，提高下载速度
    * 保存一些公司自主的jar包
31. 私服仓库配置可以直接在`settings.xml`中修改`<mirror>`标签
32. 配置发布的快照版本：在`pom.xml`中添加`<snapshotRepository>`标签就是用来配置把这个jar包发布到哪里（比如：私服仓库），发布的是一个快照版本；`<releaseRepository>`标签就是用来配置把这个jar包发布到哪里（比如：私服仓库），发布的是一个正式版本
33. Maven是通过`pom.xml`中的`<version>`来判断是上传到快照版本还是正式版本，如果是快照版本，就需要在`<version>`中添加`-SNAPSHOT`，比如正式版本是`1.0.0`，那么快照版本就是`1.0.0-SNAPSHOT`，此时就会上传到快照目录里面，然后就会上传到已经在`pom.xml`中配置好的快照版本的仓库上传地址
34. 正式版本的jar包一般是不会被覆盖的，快照版本是会被覆盖的（只要版本号一致就行）